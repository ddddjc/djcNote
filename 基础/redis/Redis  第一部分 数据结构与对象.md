...
- 哈希算法及特点
	...
	- 渐进式rehash
		...
		- 渐进式rehash的1好处：采用分而治之的方式，将rehash键值对所需的计算工作均摊到对字典每个增删改查操作上，从而避免了集中式rehash带来的庞大计算量（）
			- 在 Redis 中，rehash 的过程采用了分而治之的策略，将键值对的迁移工作均摊到每个增删改查操作上。
			- 具体实现上，Redis 使用了一个标志位 `dict.is_rehashing` 来表示是否正在进行 rehash 过程。当开始进行 rehash 时，将该标志位设置为 true。在进行增删改查操作时，Redis 会先检查 `dict.is_rehashing` 的状态。如果它为 true，表示正在进行 rehash，那么会同时对旧哈希表和新哈希表进行操作。
			- 具体的步骤如下：
				1. 如果要操作的键所在的索引位置在新哈希表中存在，那么直接在新哈希表中进行增删改查操作。
				2. 如果要操作的键所在的索引位置在旧哈希表中存在，那么先在旧哈希表中进行增删改查操作，然后再在新哈希表中进行相同的操作。
			- 这样做的目的是确保在 rehash 过程中，旧哈希表和新哈希表都可以正确处理键值对的操作。通过将增删改查操作同时应用于旧哈希表和新哈希表，可以保持数据的一致性，避免丢失或错误地处理键值对。
			- 由于增删改查操作在旧哈希表和新哈希表上同时进行，因此 rehash 的计算工作会被均摊到每个操作上。这样就实现了对计算工作的分而治之，避免了一次性大量迁移的负担，提高了整体的性能和效率。